// Description: This file contains the code to create a 3D viewer using OpenCascade Technology (OCCT) and display a 3D shape. The code also includes functions to set the shading model, material, transparency, background color, object color, edge thickness, directional light, and selection style of the displayed shape. The code also includes a timer to measure the time taken to execute the code.
...
int main()
{
    Timer t;
    MyViewer v;
    v.SetShading(Graphic3d_TypeOfShadingModel_Gouraud);
    v.SetShapeMaterial(Graphic3d_NOM_GLASS, 0);
